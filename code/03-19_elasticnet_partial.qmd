...
First, let's specify:  
  ...
  - which **engine** we want to use : Tells us which package we use 
...
```
Why 212 observations?  It is test set
...
```
How does metrics on test compare to metrics on train?  #RMSE IS LOWER AND RSQ IS HIGHER.

Why? - WE TRAINED THE MODEL ON TRAIN SET, SO METRICS ARE BETTER FOR TRAIN SET
...
final_fit %>%
  ...
  ggplot(aes(x = strength_gtex,
             y = .pred)) + ## Be able to identify what we used for x and y axis from plot for exam
  geom_point() +
  geom_abline() + ### Remember this function for exam!
...
final_spec %>%
  fit(strength_gtex ~ .,
         data = bake(weather_prep, weather)) %>%
    vi() %>% #For variables importance, remember for exam
  mutate(
